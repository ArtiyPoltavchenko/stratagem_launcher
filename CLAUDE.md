...
Версия проекта хранится **только** в `_version.py` (корень репозитория):

```python
__version__ = "0.1.2"        # стабильный релиз
__version__ = "0.1.3-rc1"    # release candidate
```

### Схема MAJOR.MINOR.PATCH[-rcN]
| Часть | Когда менять |
|-------|-------------|
| MAJOR | Ломающее изменение: переделка API, смена формата данных, несовместимость с предыдущей версией |
| MINOR | Значимая новая фича (новая категория стратагем, новый режим, новый экран) |
| PATCH | Мелкий фикс, правка данных, косметика, рефакторинг без изменения поведения |
| -rcN  | Release candidate — добавляй при выпуске нестабильной версии для тестирования. Убирай суффикс после подтверждения стабильности |

### Правило для Claude Code
**Обновляй `_version.py` при каждом коммите, который меняет поведение продукта.** Примеры:

- Добавил новую стратагему / исправил код → PATCH (`0.1.2` → `0.1.3`)
- Добавил фичу (dual-range slider, loadout delete) → MINOR (`0.1.2` → `0.2.0`)
- Сломал обратную совместимость (переименовал API endpoint, сменил формат JSON) → MAJOR (`0.1.2` → `1.0.0`)
- Выпускаешь нестабильную версию для тестирования → добавь `-rc1` (`0.2.0-rc1`)
- Версия подтверждена стабильной → убери суффикс (`0.2.0-rc1` → `0.2.0`)

Имя собранного .exe автоматически берёт версию из `_version.py`:
```
dist\Stratagem Launcher v0.1.2.exe
```

Не редактируй версию в других местах — только `_version.py`.
